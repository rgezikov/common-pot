...
- `/pot <invite_token>` — link this group chat to an existing pot from the web UI; one pot per chat — if the chat is already linked, `/pot unlink` first
- `/pot unlink` — unlink the current pot from this group chat
